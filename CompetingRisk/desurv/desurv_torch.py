...
class CondODENet(nn.Module):
    ...
    def forward(self, x, horizon):
        # cast to float to avoid error in torch.matmul
        tau = torch.matmul((horizon.unsqueeze(-1) / 2).to(torch.float32), (1 + self.u_n).to(torch.float32)) # N x n (+ 1 to push integral in 0 2 and /2 to push in 0 - t)
