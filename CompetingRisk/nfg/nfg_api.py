...
from nfg.nfg_torch import NeuralFineGrayJointTorch, NeuralFineGrayTorch
...
class NeuralFineGray(DSMBase):
  ...
  def fit(self, x, t, e, vsize = 0.15, val_data = None,
    ...
    if isinstance(x_train, dict):
      input_dim = {k: x_train[k].size(1) for k in x_train}
    else:
      input_dim = x_train.size(1)
    model = self._gen_torch_model(input_dim, optimizer, risks = maxrisk)

    model, speed = train_nfg(model, self.loss,
                         x_train, t_train, e_train,
                         x_val, t_val, e_val, cuda = self.cuda > 0,
                         **args)
  ...
  def compute_nll(self, x, t, e):
    ...
    if self.cuda > 0:
      if isinstance(x_val, dict):
        x_val = {k: x_val[k].cuda() for k in x_val}
      else:
        x_val = x_val.cuda()
  ...
  def predict_survival(self, x, t, risk = 1):
    ...
    if self.fitted:
      ...
      if isinstance(x, dict):
            # Use the first modality's number of rows.
            n_samples = x[next(iter(x))].shape[0]
            # Get device from one modality.
...
class NeuralFineGrayJoint(NeuralFineGray):

  def _gen_torch_model(self, inputdim, optimizer, risks):
    model = NeuralFineGrayJointTorch(inputdim, **self.params,
                                          risks = risks,
                                          optimizer = optimizer).double()
    if self.cuda > 0:
      model = model.cuda()
