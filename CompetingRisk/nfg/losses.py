...
def total_loss(model, x, t, e, contrastive_weight=0.0):
  # Go through network
  ...
  survival_loss = error / len(t)
  

    # ---- Contrastive loss, if needed ----
  if contrastive_weight > 0.0:
        labels = (e > 0).long()             # 0 if e=0, 1 if e>0
        features = x_rep.unsqueeze(1)       # shape [bs, 1, embed_dim]
